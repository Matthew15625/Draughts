      ...
      for x1 in x:
        if ValidMove(Board, row+y1, column+x1):
          ...
          if Board[row+y1][column+x1] == SPACE:
            ...
            jumps = getJumps(Board, row, column, x, y, NextPlayer, name)
            doubleJumps = []
            for jump in jumps:
              doubleJumps.extend(getJumps(Board, jump.NewRow, jump.NewColumn, x, y, NextPlayer, name))
            jumps = doubleJumps if len(doubleJumps) > 0 else jumps
            validMoves.extend(jumps)
    for move in validMoves:
      try:
        if move not in ListOfMoves:
          ListOfMoves[moveIndex] = move
          moveIndex += 1
      except:
        break
  return ListOfMoves

def getJumps(Board, row, column, x1, y1, NextPlayer, name):
  jumps = []
  for x in x1:
    for y in y1:
      try:
        if ValidMove(Board, row+y, column+x) and NextPlayer.lower() not in Board[row+y][column+x].lower() and Board[row+y][column+x] != SPACE:
          if Board[row+y+y][column+x+x] == SPACE:
            if not(row+y*2 == row and column+x*2 == column):
              jumps.append(MoveRecord(name, row+y+y, column+x+x, True))
      except:
        pass
  return jumps
