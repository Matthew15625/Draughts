...
class MoveRecord:
  ...
  def __init__(self, Pieces="", NewRow=-1, NewColumn=-1, CanJump=False):
    self.Pieces = Pieces
    self.NewRow = NewRow
...
def AddPlayer(Board, Player, PlayerName):
  ...
  for index, piece in enumerate(Player[1:]):
    ...
    if piece[ROW] != -1:
      ...
      Board[piece[ROW]][piece[COLUMN]] = (PlayerName.lower() if piece[QUEEN] == 0 else PlayerName.upper()) + str(index+1)
  return Board
...
def ValidMove(Board, NewRow, NewColumn):
  try:
    return Board[NewRow][NewColumn] == SPACE
  except:
    return False
