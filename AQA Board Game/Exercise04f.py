...
def AddPlayerA(Board, A):
  ...
  for index, piece in enumerate(A[1:]):
    # Make sure the piece hasn't been captured
    if piece[ROW] != -1:
      #If the piece isn't a queen, use a lowercase a. Otheriwse, use an uppercase A
      #Then add the piece number to the string, and assign it to the correct place in the board
      Board[piece[ROW]][piece[COLUMN]] = ("a" if piece[QUEEN] == 0 else "A") + str(index+1)
  return Board
